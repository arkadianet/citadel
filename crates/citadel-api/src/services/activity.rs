//! Protocol activity feed queries.
...
pub async fn get_protocol_activity(
...
) -> ServiceResult<Vec<ProtocolInteraction>> {
    ...
    let (sigma_activity, dexy_gold_activity, dexy_usd_activity, gold_lp, usd_lp, amm_activity) = tokio::join!(
        sigma_fut,
        dexy_gold_fut,
        dexy_usd_fut,
        dexy_gold_lp_fut,
