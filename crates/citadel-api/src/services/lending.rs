//! Duckpools lending use-case orchestration: markets/positions fetch, build_* txs, proxy refunds.

use crate::dto::lending::{
...
pub async fn get_markets(state: &AppState) -> ServiceResult<MarketsResponse> {
...
pub async fn get_positions(state: &AppState, address: String) -> ServiceResult<PositionsResponse> {
    let client = state.require_node_client().await?;
    let capabilities = client.require_capabilities().await?;
...
) -> ServiceResult<Vec<lending_tx_builder::UserUtxo>> {
...
fn parse_single_lending_utxo(
    v: serde_json::Value,
    idx: usize,
) -> ServiceResult<lending_tx_builder::UserUtxo> {
    let box_id = v["boxId"]
        .as_str()
...
fn lending_build_response_to_dto(
    response: lending_tx_builder::BuildResponse,
) -> ServiceResult<LendingBuildResponse> {
...
pub async fn build_lend(
    ...
    lending_build_response_to_dto(result)
}
...
pub async fn build_borrow(
    ...
    let collateral_options =
        lending::fetch_collateral_from_parameter_box(&client, &capabilities, pool_config)
            .await
            .into_service()?;
...
pub async fn build_refund(
    ...
    let result =
        lending_tx_builder::build_refund_tx(proxy_box, request.current_height).into_service()?;
...
pub async fn check_proxy_box(state: &AppState, box_id: String) -> ServiceResult<serde_json::Value> {
    ...
    let ergo_box = client.get_box_by_id(&ergo_box_id).await.into_service()?;
...
pub async fn discover_stuck_proxies(
    ...
    let client = state.require_node_client().await?;
    let current_height = client.current_height().await.into_service()? as u32;
    ...
    serde_json::to_value(&stuck_boxes).into_service()
}

pub async fn get_dex_price(state: &AppState, dex_nft: String) -> ServiceResult<serde_json::Value> {
    let client = state.require_node_client().await?;
    let capabilities = client.require_capabilities().await?;

    let token_id = citadel_core::TokenId::new(&dex_nft);
    let dex_box = client
        .get_box_by_token_id(&capabilities, &token_id)
        .await
...
fn decode_sigma_int_or_long(hex_str: &str) -> Result<i64, String> {
    ...
    match bytes[0] {
        ...
        other => Err(format!(
            "Expected Int (0x04) or Long (0x05), got 0x{:02x}",
            other
        )),
    }
}
