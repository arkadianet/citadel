...
pub async fn explorer_node_info(state: &AppState) -> ServiceResult<serde_json::Value> {
    let client = state.require_node_client().await?;
    client.get_full_node_info().await.into_service()
}
...
pub async fn explorer_get_block(
    ...
    if block_id.chars().all(|c| c.is_ascii_digit()) {
        ...
        client.get_block_by_id(header_id).await.into_service()
    } else {
        client.get_block_by_id(&block_id).await.into_service()
    }
}
...
pub async fn explorer_get_mempool(state: &AppState) -> ServiceResult<Vec<serde_json::Value>> {
    let client = state.require_node_client().await?;
    client.get_mempool_transactions().await.into_service()
}
...
pub async fn explorer_get_address(
    ...
    let (erg_balance, tokens) = balance_result.into_service()?;
    let (transactions, total_txs) = txs_result.into_service()?;
...
pub async fn explorer_search(state: &AppState, query: String) -> ServiceResult<serde_json::Value> {
    let client = state.require_node_client().await?;
    let q = query.trim();
