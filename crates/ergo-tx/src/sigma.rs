...
/// Encode an i32 as a Sigma Int register value (hex string).
...
/// Encode an i64 as a Sigma Long register value (hex string).
/// Format: 0x05 (SLong type tag) + VLQ zigzag encoded value
pub fn encode_sigma_long(value: i64) -> String {
    let mut bytes = vec![0x05u8];
    // Zigzag encode — bitwise form handles i64::MIN without overflow
    let zigzag = ((value << 1) ^ (value >> 63)) as u64;
    vlq_encode(&mut bytes, zigzag);
    hex::encode(bytes)
}
...
/// Encode a GroupElement (compressed EC point) as a Sigma register value.
/// Format: 0x07 (SGroupElement) + 33-byte compressed EC point (02/03 prefix + X coord)
...
/// Format: 0x59 (tuple type) + two bare zigzag-VLQ Longs (no 0x05 type tags).
pub fn encode_sigma_long_pair(a: i64, b: i64) -> String {
    let mut bytes = vec![0x59u8];
...
/// Extract the 33-byte compressed public key from a P2PK ErgoTree (`0008cd` + 33 bytes).
...
/// Decode a GroupElement from a Sigma register hex string (strips 0x07 type tag).
...
/// Encode a `Coll[SByte]` as a Sigma register hex string.
/// Format: 0x0e (Coll[SByte]) + VLQ length + raw bytes
pub fn encode_sigma_coll_byte(data: &[u8]) -> String {
    let mut bytes = vec![0x0eu8];
    vlq_encode(&mut bytes, data.len() as u64);
    bytes.extend_from_slice(data);
    hex::encode(bytes)
}

/// Encode a `Coll[Coll[SByte]]` as a Sigma register hex string.
/// Format: 0x0e 0x0c (Coll[Coll[SByte]] type) + VLQ count + length-prefixed byte arrays.
/// Type descriptor: 0x0e = Coll with nested type, 0x0c = Coll[SByte] inner type.
...
#[macro_export]
macro_rules! sigma_registers {
    ($($reg:expr => $val:expr),+ $(,)?) => {{
        let mut _regs = ::std::collections::HashMap::<String, String>::new();
        $(_regs.insert($reg.to_string(), $val);)+
        _regs
    }};
}
...
mod tests {
    ...
    fn test_vlq_encode_large_length() {
        ...
        // VLQ for 200: 0xC8 0x01 (200 & 0x7F = 0x48 | 0x80, 200 >> 7 = 1)
