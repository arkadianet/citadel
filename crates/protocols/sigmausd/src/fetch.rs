...
use ergo_tx::ergo_box_utils::{get_register_long, map_node_error};
...
pub async fn fetch_sigmausd_state(
    ...
    let bank_box = client.get_box_by_token_id(capabilities,
        &bank_token_id,
    )
    .await
...
pub(crate) fn parse_bank_box(ergo_box: &ErgoBox) -> Result<BankBoxData, ProtocolError> {
    ...
    let sigusd_circulating = get_register_long(ergo_box, NonMandatoryRegisterId::R4)?;
    let sigrsv_circulating = get_register_long(ergo_box, NonMandatoryRegisterId::R5)?;
...
pub(crate) fn parse_oracle_box(ergo_box: &ErgoBox) -> Result<OracleBoxData, ProtocolError> {
    ...
    let box_id = BoxId::new(ergo_box.box_id().to_string());
    let nanoerg_per_usd = get_register_long(ergo_box, NonMandatoryRegisterId::R4)?;
...
pub async fn fetch_tx_context(
    ...
    let bank_tx_id = client.get_box_creation_info(&bank_box.box_id().to_string(),
    )
    .await
    .map_err(|e| ProtocolError::BoxParseError {
