...
pub fn parse_bank_box(ergo_box: &ErgoBox, ids: &DexyIds) -> Result<DexyBankBoxData, ProtocolError> {
    ...
    let ergo_tree = serialize_ergo_tree(ergo_box)?;
...
/// R4: Long = nanoERG per unit of underlying asset
pub fn parse_oracle_box(ergo_box: &ErgoBox) -> Result<DexyOracleBoxData, ProtocolError> {
    use ergo_lib::ergotree_ir::chain::ergo_box::NonMandatoryRegisterId;

    let box_id = ergo_box.box_id().to_string();
    let rate_nano = get_register_long(ergo_box, NonMandatoryRegisterId::R4)?;
...
/// R4: Int = reset height, R5: Long = remaining tokens available this period
pub fn parse_free_mint_box_data(ergo_box: &ErgoBox) -> Result<DexyFreeMintBoxData, ProtocolError> {
    use ergo_lib::ergotree_ir::chain::ergo_box::NonMandatoryRegisterId;

    let box_id = ergo_box.box_id().to_string();
    let reset_height = get_register_int(ergo_box, NonMandatoryRegisterId::R4)?;
    let available = get_register_long(ergo_box, NonMandatoryRegisterId::R5)?;
...
/// FreeMint transaction context.
/// Inputs: [0] FreeMint, [1] Bank, [2] Buyback
/// Data Inputs: [0] Oracle, [1] LP
#[derive(Debug, Clone)]
pub struct DexyTxContext {
    ...
    /// nanoERG per USD (raw, before decimal adjustment)
...
pub async fn fetch_tx_context(
    ...
    let free_mint_data = parse_free_mint_box_data(&free_mint_box)?;
    let free_mint_ergo_tree = serialize_ergo_tree(&free_mint_box)?;
    let bank_data = parse_bank_box(&bank_box, ids)?;
    let buyback_ergo_tree = serialize_ergo_tree(&buyback_box)?;
    let oracle_data = parse_oracle_box(&oracle_box)?;
    let lp_data = parse_lp_box(&lp_box, ids)?;
    ...
    Ok(DexyTxContext {
        ...
        free_mint_ergo_tree,
        free_mint_r4_height: free_mint_data.reset_height,
        free_mint_r5_available: free_mint_data.available,
        free_mint_box,
        bank_input,
...
/// LP swap transaction context.
/// Inputs: [0] LP, [1] Swap NFT, [2+] User UTXOs
/// Outputs: [0] LP (updated), [1] Swap NFT (preserved), [2] User output, [3+] Change/Fee
#[derive(Debug, Clone)]
...
pub async fn fetch_swap_tx_context(
    ...
    let swap_ergo_tree = serialize_ergo_tree(&swap_box)?;
...
/// LP deposit/redeem transaction context.
/// Deposit: Inputs [LP, MintNFT, UserUTXOs], no data inputs
/// Redeem: Inputs [LP, RedeemNFT, UserUTXOs], Data Inputs [Oracle]
#[derive(Debug, Clone)]
...
pub async fn fetch_lp_tx_context(
    ...
    let action_ergo_tree = serialize_ergo_tree(&action_box)?;
